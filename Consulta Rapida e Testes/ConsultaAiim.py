...
def normalizar_descricao(desc):
    """Remove texto após palavras-chave específicas para normalizar a descrição."""
...
aiims_invalido = {"AIIM liquidado", "Protocolo da Defesa",
            ...
            try:
                ...
                if ERRO == "":
                    ...
                    if DRT in outros:
                            ...
                            for col in range(1, 12):
                                sheet.cell(row=linha_planilha, column=col).fill = cor_outros
                        # Verifica a descrição se DRT não estiver em outros
                else:
