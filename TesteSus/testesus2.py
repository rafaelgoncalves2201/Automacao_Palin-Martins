...
print("Processo de Consulta no Epat sendo realizado")
        ...
        try:
            ...
            try:
                ...
                if ERRO == "":
                    ...
                    def diferenca_data(DATE, DATA):
                        data_atual = DATE
                        data_fornecida = DATA
                        diferenca = data_atual - data_fornecida
                        return diferenca > timedelta(days=90)
                    ...
                    elif DESC in aiims_verifi and diferenca_data(DATA):
                        sheet.cell(row=linha_planilha, column=11).value = "Passar ClickUp"
                        for col in range(1, 12):
                            sheet.cell(row=linha_planilha, column=col).fill = cor_clickup
                    else:
